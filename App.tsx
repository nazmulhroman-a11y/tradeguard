...
function App() {
  ...
  const [marketSentiment, setMarketSentiment] =
    useState<MarketSentiment>("NEUTRAL");
  ...
  useEffect(() => {
    ...
    const generateInsights = () => {
      ...
      const profitPercentage =
        ((currentBalance - settings.initialBalance) / settings.initialBalance) *
        100;

      const last5Trades = trades.slice(-5);
      const consecutiveWins = last5Trades
        .reverse()
        .findIndex((t) => t.result === "LOSS");
      const currentWinStreak =
        consecutiveWins === -1 ? last5Trades.length : consecutiveWins;

      const last5TradesOriginal = trades.slice(-5);
      const consecutiveLosses = last5TradesOriginal
        .reverse()
        .findIndex((t) => t.result === "WIN");
      const currentLossStreak =
        consecutiveLosses === -1
          ? last5TradesOriginal.length
          : consecutiveLosses;

      // Calculate Recent Win Rate (Last 5)
      const recentWins = last5TradesOriginal.filter(
        (t) => t.result === "WIN"
      ).length;
      ...
      if (trades.length >= 5) {
        if (winRate < 45 && settings.strategy === "PERCENTAGE") {
          newInsights.push({
            ...
            message:
              "Data from 5,000 users with similar ~40% win rates shows that the Masaniello strategy yielded 15% more profit than your current Percentage strategy.",
            severity: "info",
            timestamp: Date.now(),
      ...
      if (trades.length >= 8 && settings.strategy === "MASANIELLO") {
        ...
        if (allWins && winRate > 60) {
          ...
          newInsights.push({
            ...
            message:
              "Time-series analysis predicts a continuation of the bullish trend for the next 3 trades. AI suggests temporarily increasing your Masaniello target to capture this.",
            severity: "success",
            timestamp: Date.now(),
            ...
            onAction: () => {
              setSettings((prev) => ({
                ...prev,
                targetWins: Math.min(prev.totalEvents, prev.targetWins + 1),
              }));
            },
          });
        }
      ...
      if (settings.enableAutoPilot) {
        ...
        if (recentWinRate >= 80 && settings.strategy === "FIXED") {
          newInsights.push({
            ...
            message:
              'You won 4 out of 5 trades. AI recommends switching to "Percentage" mode to compound these gains.',
            severity: "success",
            timestamp: Date.now(),
        ...
        if (recentWinRate <= 20 && settings.strategy === "PERCENTAGE") {
          newInsights.push({
            ...
            message:
              'Recent performance is low. AI strongly suggests switching to "Fixed Amount" to prevent rapid drawdown.',
            severity: "warning",
            timestamp: Date.now(),
            ...
            onAction: () => {
              setSettings((prev) => ({
                ...prev,
                strategy: "FIXED",
                fixedAmount: prev.initialBalance * 0.01,
              })); // Default 1% fixed
      ...
      if (trades.length >= 6) {
        ...
        const isAlternating = last6.every(
        ...
        if (isAlternating) {
          newInsights.push({
            ...
            message:
              "W/L/W/L/W/L pattern detected. This often indicates market manipulation or a bot algorithm. AI suggests pausing.",
            severity: "critical",
            timestamp: Date.now(),
          });
        }
      ...
      if (lastTrade && prevTrade) {
        if (
          prevTrade.result === "LOSS" &&
          lastTrade.investment > prevTrade.investment * 1.5 &&
          ...
          newInsights.push({
            ...
            message: `Martingale detected. -€${lastTrade.investment.toFixed(
              2
            )} loss. I can set a Safety Brake.`,
            severity: "critical",
            timestamp: Date.now(),
            ...
            onAction: () => {
              setSettings((prev) => ({
                ...prev,
                maxLossesAllowed: totalLosses + 3,
              }));
      ...
      if (profitPercentage >= 15 && totalProfit > 0) {
        newInsights.push({
          ...
          message: `+${profitPercentage.toFixed(
            1
          )}% ROI secured. Lock in profit now?`,
          severity: "warning",
          timestamp: Date.now(),
        });
      }
  ...
  const safetyChecks = {
    drawdownLimit:
      settings.maxDrawdownWarning > 0 &&
      currentDrawdown >= settings.maxDrawdownWarning,
    maxLosses:
      settings.maxLossesAllowed > 0 && totalLosses >= settings.maxLossesAllowed,
    takeProfit:
      settings.sessionTakeProfit > 0 &&
      totalProfit >= settings.sessionTakeProfit,
    stopLoss:
      settings.sessionStopLoss > 0 && totalProfit <= -settings.sessionStopLoss,
  };
  ...
  const isMasanielloComplete =
    settings.strategy === "MASANIELLO" &&
    (remainingTrades <= 0 || remainingWins <= 0);
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Smart Money & Risk Management System
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <FlaskConical className="w-4 h-4" /> AI Lab{" "}
          ...
          <div className="bg-rose-500/10 border border-rose-500/50 rounded-xl p-4 flex items-start gap-4 animate-in slide-in-from-top-2">
            ...
            <div>
              <h3 className="font-bold text-rose-400 text-lg">
                Trading Safety Lock Activated
              </h3>
              <p className="text-slate-300 text-sm mt-1">
                The system has halted trading because a safety limit was
                reached:
              </p>
              <ul className="list-disc list-inside text-slate-400 text-sm mt-2 space-y-1">
                {safetyChecks.drawdownLimit && (
                  <li>
                    Max Drawdown Limit Reached ({currentDrawdown.toFixed(2)}%)
                  </li>
                )}
                {safetyChecks.maxLosses && (
                  <li>Max Losses Limit Reached ({totalLosses} Losses)</li>
                )}
                {safetyChecks.takeProfit && (
                  <li className="text-emerald-400">
                    Target Session Profit Achieved! (+€{totalProfit.toFixed(2)})
                  </li>
                )}
                {safetyChecks.stopLoss && (
                  <li>
                    Stop Loss Limit Reached (Current: €{totalProfit.toFixed(2)})
                  </li>
                )}
              </ul>
              <button
                onClick={resetData}
                className="mt-4 text-xs bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-lg font-bold transition-colors"
              >
                ...
                >
                  <h3
                    className={`font-bold text-lg flex items-center gap-2 ${
                      masanielloSuccess ? "text-emerald-400" : "text-rose-400"
                    }`}
                  >
                    {masanielloSuccess
                      ? "🎉 Target Achieved!"
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span
                      className={`font-mono font-bold ${
                        currentDrawdown > (settings.maxDrawdownWarning || 100)
                          ? "text-rose-500"
                          : "text-slate-200"
                      }`}
                    >
                  ...
                  <div className="w-full bg-slate-700 h-2 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${
                        currentDrawdown > 5 ? "bg-orange-500" : "bg-emerald-500"
                      }`}
                      style={{
                        width: `${Math.min(currentDrawdown * 5, 100)}%`,
                      }} // Visual scaling
                    ></div>
                  </div>
                </div>
                ...
                {settings.strategy === "MASANIELLO" && (
                  <div className="pt-4 border-t border-slate-700/50 mt-4">
                    <h5 className="text-xs font-bold text-indigo-400 mb-2 uppercase">
                      Masaniello Progress
                    </h5>
